 ...
 * that's already, persistently assigned to its matching compass team. That
 * lookup is used ONLY to find the right workspace to list/embed from
 * (app/dashboard/subaddress-swap/page.js) and for this app's own preflight —
 * it is never sent back to Sigma as a `teams` claim. The signed JWT asserts
 * `sub` alone; Sigma applies the member's real team membership on its own.
 */
