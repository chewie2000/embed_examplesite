...
export async function generateSigmaEmbedUrl({
  ...
  const token = await new SignJWT({
    sub: email,            // Required: user email
    iss: clientId,         // Optional but recommended: must match CLIENT_ID
    jti: crypto.randomUUID(), // Required: unique token ID (prevents replay attacks)
    account_type: accountType || 'viewer',
    teams,
