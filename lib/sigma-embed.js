 ...
 * @param {string} [options.org]        - Optional org override, orthogonal to `mode`. `mode` picks
 *                                        which pre-configured workbook to show WITHIN the org this
 *                                        app's default credentials point at; `org` picks a DIFFERENT
 *                                        org entirely, via its own LEGACY_SIGMA_BASE_URL /
 *                                        LEGACY_SIGMA_CLIENT_ID / LEGACY_SIGMA_SECRET (all three
 *                                        required together — a client_id/secret pair is only valid
 *                                        for the org that issued it, so there's no sane fallback to
 *                                        the default org's credentials here). Currently only
 *                                        `'legacy'` is recognized (Legacy Examples, embed_examplesite
 *                                        -0so.11 — kept pointed at the org this site ran against
 *                                        before the 2026-09 org switch).
 *
...
export async function generateSigmaEmbedUrl({
  ...
  if (urlId) {
    ...
    // origin/slug as the relevant *_SIGMA_BASE_URL. Sigma routes on the
    // trailing urlId only — the name-slug prefix is cosmetic — so no
    // separate per-urlId env var is needed, just the right org's anchor URL
    // to read the origin/slug from. The Content Browser is entirely
    // Legacy-Examples-scoped (see /api/sigma/tree), so any urlId it
    // discovered belongs to the legacy org, not the default one — using the
    // wrong anchor here would silently construct a URL for the wrong org
    // and 404.
    const anchorBaseUrl = org === 'legacy' ? process.env.LEGACY_SIGMA_BASE_URL : process.env.SIGMA_BASE_URL;
    if (!anchorBaseUrl) {
      throw new Error(
        org === 'legacy'
          ? 'LEGACY_SIGMA_BASE_URL is not configured (needed to derive the org URL for content-browser embeds).'
          : 'SIGMA_BASE_URL is not configured (needed to derive the org URL for content-browser embeds).'
      );
    }
    const orgUrl = new URL(anchorBaseUrl);
    const orgSlug = orgUrl.pathname.split('/').filter(Boolean)[0];
    baseUrl = `${orgUrl.origin}/${orgSlug}/workbook/${urlId}`;
  } else if (org === 'legacy') {
    // Both Legacy Examples pages share this one workbook (the "secured"
    // example is the same workbook with a URL-param filter layered on, same
    // as it is in the default org) — no per-mode base URL needed here.
    baseUrl = process.env.LEGACY_SIGMA_BASE_URL;
    if (!baseUrl) {
      throw new Error('LEGACY_SIGMA_BASE_URL is not configured (needed for the org="legacy" override).');
    }
  } else {
  ...
  const clientId = org === 'legacy' ? process.env.LEGACY_SIGMA_CLIENT_ID : process.env.SIGMA_CLIENT_ID;
  if (!clientId) throw new Error(org === 'legacy' ? 'LEGACY_SIGMA_CLIENT_ID is not configured.' : 'SIGMA_CLIENT_ID is not configured.');

  const secret = org === 'legacy' ? process.env.LEGACY_SIGMA_SECRET : process.env.SIGMA_SECRET;
  if (!secret) throw new Error(org === 'legacy' ? 'LEGACY_SIGMA_SECRET is not configured.' : 'SIGMA_SECRET is not configured.');
