...
export async function resolveMemberByEmail(email) {
  ...
  const data = await apiGet(`/members?email=${encodeURIComponent(email)}`);
...
// ── Workspaces ──────────────────────────────────────────────────────────────
/**
 * Resolve a workspace by exact (case-insensitive) name.
 *
 * The useful field on the result is `workspaceUrlId`, not `workspaceId` — the
 * files API accepts the former as a `parentId`, and workspaces don't appear in
 * /files at all, so there's no other way to list their contents.
 */
export async function resolveWorkspaceByName(name) {
  if (!name) return null;
  const entries = await apiGetAll('/workspaces');
  const target = name.trim().toLowerCase();
  return entries.find((w) => (w.name || '').trim().toLowerCase() === target) ?? null;
}

/**
 * Find the first workbook sitting directly inside a named workspace.
 * Returns null when the workspace doesn't exist or holds no workbook.
 */
export async function findWorkbookInWorkspace(name) {
  const ws = await resolveWorkspaceByName(name);
  if (!ws) return null;
  const files = await apiGetAll(`/files?parentId=${encodeURIComponent(ws.workspaceUrlId)}`);
  const wb = files.find((f) => f.type === 'workbook');
  return wb ? { name: wb.name, urlId: wb.urlId, workspace: ws.name } : null;
}
