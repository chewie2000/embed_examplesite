 ...
 * Returns null when no workbook is found there.
 *
...
export async function findWorkbookInWorkspace(name) {
  ...
  const wb = entries.find((w) => (w.path || '').trim().toLowerCase() === target);
  return wb ? { name: wb.name, urlId: wb.workbookUrlId, workspace: name } : null;
}
