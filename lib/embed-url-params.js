 ...
 *
 * 'secured' mode applies the per-user region filter to the legacy org's
 * EmbedProfitPlanner workbook (LEGACY_SIGMA_BASE_URL), via its "Region" list
 * control:
 ...
 *
 * The URL parameter key MUST be the control's `controlId` (an identifier
 * the workbook builder sets, independent of the control's display name/label
 * shown in the Sigma UI) — NOT the display name, and — per
 * https://help.sigmacomputing.com/docs/workbook-control-values-in-the-url —
 * with no leading colon (colon-prefixed params are reserved for Sigma's own
 * embed settings like :hide_menu). Confirmed live via
 * GET /v2/workbooks/{id}?includeContents=true: this control's display name
 * is "Region" but its controlId is "F_Region" — the two are NOT
 * interchangeable, which is exactly what made the previous 'Store-Region'/
 * 'Store-Region-Page' keys silently no-op (neither is this control's real
 * controlId, and Sigma has no "-Page"-suffixed control-targeting concept at
 * all — the single control here already filters every table it's wired to,
 * regardless of which page(s) they appear on).
 */
/**
